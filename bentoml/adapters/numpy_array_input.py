...
class NumpyArrayInput(BaseInputAdapter):
    """NumpyArrayInput expects inputs from HTTP request or cli arguments that
        can be converted into a pandas numpy array. It passes down the numpy array
    ...
    Args:
        ...
        type (str): Type of object to recover for read json with pandas. Default is
            frame
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        if not is_batch_input:
            raise ValueError('NumpyArrayInput can not accept none batch inputs')
        ...
        global np  # pylint: disable=global-statement
        if np is None:
            try:
    ...
    @property
    def config(self):
        base_config = super(NumpyArrayInput, self).config
        return dict(
            base_config,
            orient=self.orient,
            typ=self.typ,
